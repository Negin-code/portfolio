...
const ProjectCard = ({
  ...
  return (
    ...
    >
      {/* Main card container */}
      <div
        className={`relative grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start min-h-[800px]`}
      >
        {/* Image Section */}
        <figure
          className={`
        ...
        >
          {/* Glass overlay */}
          <div
            className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent backdrop-blur-[2px] z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
            aria-hidden="true"
          />
          ...
          <div
            className="absolute inset-0 bg-gradient-to-t from-[#493B32]/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-20"
            aria-hidden="true"
          />
        </figure>
          ...
          <div className="relative backdrop-blur-md bg-white/80 dark:bg-gray-900/80 rounded-2xl p-6 md:p-8 border border-white/50 shadow-2xl">
            ...
            <header className="mb-6">
              ...
              <h3
                ...
                className="text-2xl md:text-3xl font-tan text-[#493B32] dark:text-white mb-4"
              >
                {title}
              </h3>
            ...
            {/* Technologies */}
            <div className="mb-6" role="group" aria-label="Technologies used">
              <ul className="flex flex-wrap gap-2" role="list">
                {technologies.map((tech, idx) => (
                  <li key={idx} role="listitem">
                    <span className="px-3 py-1.5 text-sm rounded-full bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-[#493B32]/20 dark:border-gray-600/20 text-[#493B32] dark:text-gray-300">
                      {tech}
                    </span>
                  </li>
                ))}
